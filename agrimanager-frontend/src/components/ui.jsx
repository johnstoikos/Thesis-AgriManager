...
export function Button({
  ...
  const variantClasses = {
    ...
    secondary: "bg-white text-slate-700 border border-slate-200 shadow-sm hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-md",
...
export function Surface({ children, className = "" }) {
  return (
    <div
      className={cn(
        "rounded-3xl border border-white/60 bg-white/70 shadow-[0_18px_45px_rgba(15,23,42,0.08)] backdrop-blur-md",
        className
      )}
    >
      {children}
...
export function StatCard({ icon: Icon, title, value, tone = "emerald", helper }) {
  ...
  return (
    <Surface className="p-5 transition hover:-translate-y-0.5 hover:shadow-[0_22px_50px_rgba(15,23,42,0.12)]">
      ...
      <p className="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-slate-500">{title}</p>
    ...
    <div className={cn("animate-pulse space-y-3", className)}>
      {Array.from({ length: lines }).map((_, index) => (
        <div
          key={index}
          className={cn(
            "h-4 rounded-full bg-slate-200/90",
  ...
  return (
    <Surface className={cn("p-5 md:p-6", className)}>
      <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 md:flex-row md:items-start md:justify-between">
        <div>
          {badge && (
            <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-slate-600">
              {badge}
            </span>
          )}
...
export function ModalShell({ title, description, onClose, children, className = "", size = "xl" }) {
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
      <div className={cn("w-full overflow-hidden rounded-[28px] border border-white/40 bg-white shadow-2xl duration-300 animate-[modal-pop_0.22s_ease-out]", sizeClasses[size], className)}>
...
export function FieldInput({ className = "", ...props }) {
  return (
    <input
      className={cn(
        "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-emerald-300 focus:ring-2 focus:ring-emerald-300/60",
        className
      )}
      {...props}
    />
...
export function FieldSelect({ className = "", children, ...props }) {
  return (
    <select
      className={cn(
        "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-300/60",
        className
      )}
      {...props}
    >
